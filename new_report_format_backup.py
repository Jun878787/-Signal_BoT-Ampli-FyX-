...
def format_new_group_report(transactions: List[Dict], group_name: str = "群組", db_manager=None) -> str:
    ...
    try:
        ...
        # Convert all transaction amounts to safe floats upfront
        for t in transactions:
            if 'amount' in t:
                t['amount'] = safe_float(t['amount'])
        ...
        for day_key in sorted(daily_transactions.keys()):
            ...
            tw_daily = sum(safe_float(t['amount']) for t in day_trans if t['currency'] == 'TW' and t['type'] == 'income')
            cn_daily = sum(safe_float(t['amount']) for t in day_trans if t['currency'] == 'CN' and t['type'] == 'income')
